...
content_id: 4a7ea27071734bc0230128927b5780074192be2cfab68caf07b66bdddedcec56
---
...
export default class LabelConcept {
  ...
  async discard(
    a:
      | Id
      | { user_id: Id; tx_id: Id; tx_name?: unknown; tx_merchant?: unknown },
    b?: Id,
    c?: string,
    d?: string,
  ): Promise<{ label_tx_id: Id }> {
    ...
    // Delegate to stage for the centralised checks and insertion, using TRASH_CATEGORY_ID
  ...
  async get_tx_in_trash(payload: { user_id: Id }): Promise<string[]>;
  async get_tx_in_trash(a: Id | { user_id: Id }): Promise<string[]> {
    const user_id = a instanceof Id ? a : a.user_id;
  ...
  async getStagedLabels(payload: { user_id: Id }): Promise<StagedLabelDoc[]>;
  async getStagedLabels(a: Id | { user_id: Id }): Promise<StagedLabelDoc[]> {
    const user_id = a instanceof Id ? a : a.user_id;
  ...
  async suggest(
    ...
    const user_id = a instanceof Id ? a : a.user_id;
    const allCategories = a instanceof Id ? b! : a.allCategories;
    const txInfo = a instanceof Id ? c! : a.txInfo;
