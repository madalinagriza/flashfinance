...
content_id: 28f929b5a0234051a79b2a5389923be580d2e8a8d4408dad421dd8b2bda06f73
---
...
export const GetUnlabeledTransactionsRequest: Sync = ({
  ...
  where: async (frames) => {
    const originalFrame = frames[0];
    console.log("[GetUnlabeledTransactions] where start", {
      request: originalFrame[request],
    });

    frames = await frames.query(Sessioning._getUser, { session }, { user });
    if (frames.length === 0) {
      console.log("[GetUnlabeledTransactions] no user for session", {
        session: originalFrame[session],
      });
      return new Frames({ ...originalFrame, [error]: "Unauthorized" });
    }

    console.log("[GetUnlabeledTransactions] querying transactions", {
      owner: frames[0][user],
    });
    ...
    console.log(
      "[GetUnlabeledTransactions] transactionFrames length",
      transactionFrames.length,
    );
