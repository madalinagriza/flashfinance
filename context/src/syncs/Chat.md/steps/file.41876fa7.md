...
content_id: 41876fa7c518c777003482c1d5c0be0552b02c0e2f007d73281366a058ebc2ae
---

# file: src\concepts\Transaction\TransactionConcept.ts

I'll update two methods in this file. First, `getTransaction` will be made more robust by returning an empty array instead of throwing an error when a transaction isn't found. Second, `getTxInfo` will be updated to return the data in the correct shape.
...
export default class TransactionConcept {
  ...
  async getTransaction(
    ...
    if (!tx) {
      // Return empty array if not found, which is more robust for queries.
      return [];
    }
    return [{ tx: tx }];
  }
  ...
  async getTxInfo(
    ...
    // Map the wrapped TransactionDoc and wrap the result to match sync expectations.
    return txs.map(({ tx }) => ({
      txInfo: {
        date: tx.date,
        merchant_text: tx.merchant_text,
