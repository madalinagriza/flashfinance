---
timestamp: 'Fri Nov 07 2025 04:21:16 GMT-0500 (Eastern Standard Time)'
parent: '[[..\20251107_042116.25a8eebf.md]]'
content_id: 7e38909370997e16261d621221bfdd76f1e0c7799ffdba2ab6a67daee458b940
---

# response:

Based on the logs and the provided code, the problem is a mismatch between the data structure returned by the `Transaction.getTxInfo` query and the data structure expected by the `GetTxInfoRequest` synchronization that calls it. This mismatch causes a runtime error when the synchronization tries to access a `.length` property on a value that is `undefined`.

Here is a step-by-step breakdown of the issue:

1. **The Query's Return Value**: In `src/concepts/Transaction/TransactionConcept.ts`, the `getTxInfo` method has the following implementation:
   ```typescript
   // in TransactionConcept.ts
   async getTxInfo(
     // ...
   ...
   ```
   As the logs confirm, this method correctly returns an array containing a single plain object, like `[{ date: ..., merchant_text: ..., amount: ... }]`.

2. **The Synchronization's Expectation**: In `src/syncs/transaction.sync.ts`, the `GetTxInfoRequest` sync calls this query using the following pattern:
   ```typescript
   // in transaction.sync.ts
   const infoFrames = await frames.query(
     Transaction.getTxInfo,
     { owner_id: user, tx_id },
     { txInfo }, // This is shorthand for { txInfo: txInfo }
   );
   ```
   The output pattern `{ txInfo }` instructs the synchronization engine to look for a property named `txInfo` inside each object returned by the query. It then binds the *value* of that property to the `txInfo` variable in the frame.

3. **The Mismatch**: Since the objects returned by `getTxInfo` (`{ date: ... }`) do not have a `txInfo` property, the engine binds `undefined` to the `txInfo` variable.

4. **The Error**: The `where` clause then attempts to execute this code:
   ```typescript
   // in transaction.sync.ts
   if (
     infoFrames.length === 0 ||
     (infoFrames[0][txInfo] as unknown[]).length === 0 // <-- ERROR HERE
   ) {
     // ...
   }
   ```
   Because the `txInfo` variable in the frame is `undefined`, this line becomes `(undefined).length`, which throws the error: `Cannot read properties of undefined (reading 'length')`.

### Solution

To fix this, you need to correct both the concept's query method to return the expected structure and the synchronization's logic to correctly handle the result.

#### 1. Update the `Transaction.getTxInfo` Method

Modify `getTxInfo` in `src/concepts/Transaction/TransactionConcept.ts` to wrap its result in an object with a `txInfo` key, which is the standard pattern for concept queries.
...
#### 2. Simplify the `GetTxInfoRequest` Synchronization

Now that the concept returns the correct structure, the `where` clause in `src/syncs/transaction.sync.ts` can be greatly simplified. The complex and incorrect logic can be removed.
...
export const GetTxInfoRequest: Sync = (
  ...
  where: async (frames) => {
    ...
    // Query for the transaction info. The `txInfo` variable will be bound to the info object.
    const infoFrames = await frames.query(
      Transaction.getTxInfo,
      { owner_id: user, tx_id },
      { txInfo },
    ...
    // Handle transaction not found. The query will result in empty frames if not found.
    if (infoFrames.length === 0) {
      return new Frames({ ...originalFrame, [error]: "Transaction not found" });
    }

    // The variable is already correctly bound to the object. No mapping needed.
    return infoFrames;
  },
