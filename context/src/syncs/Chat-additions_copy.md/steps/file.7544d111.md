...
content_id: 7544d111f7d5edf064f6caf3cd5e8fe4e4e340432f45e01d438dbfb93485776c
---
...
export default class CategoryConcept {
  ...
  async addTransaction(
    ...
    const txExists = metricDoc.transactions.some((entry) => entry.tx_id === txIdStr);

    if (txExists) {
      // This is not an error, the operation is idempotent. Return success.
      return { ok: true };
    }
  ...
  async bulk_add_transaction(
    ...
    try {
      ...
      throw new Error(`One or more transactions failed to be added. First error: ${(error as Error).message}`);
    }
  }
}
```
