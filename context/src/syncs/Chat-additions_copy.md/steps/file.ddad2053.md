...
content_id: ddad20534bbb12c6d663e275f05f69a93bb208b623b425ebca6af93fb1ed7eda
---
...
export default class TransactionConcept {
  ...
  async mark_labeled(
    ...
    if (existingTx.status === TransactionStatus.LABELED) {
      // This is not an error, just idempotent. We can return success.
      return { tx_id: tx_id };
    }
  ...
  async bulk_mark_labeled(
    ...
    try {
      ...
      throw new Error(`One or more transactions failed to be marked as labeled. First error: ${(error as Error).message}`);
    }
  }
}
```

### 2. `Category.bulk_add_transaction` Implementation

Similarly, this method is added to `CategoryConcept`. It takes a list of transaction entries, maps them to parallel `addTransaction` calls, and awaits them with `Promise.all`.
