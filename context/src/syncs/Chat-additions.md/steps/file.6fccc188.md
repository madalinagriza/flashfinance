...
content_id: 6fccc1884ac199ae8001d92dd4dbfe10097b2ee62ed7f53501cde4bbe1a383c1
---
...
// NOTE: This relative import is necessary because the `Period` class is defined inside the concept file
// and is required by the `_getMetric` query. A cleaner design might export such types at the top level
// or have the query accept primitive types.
...
export const GetCategoryNameByIdRequest: Sync = (
  { request, session, category_id, user, name, error },
) => ({
  when: actions([Requesting.request, {
    path: "/Category/getCategoryNameById",
    session,
    category_id,
  }, { request }]),
  where: async (frames) => {
    ...
    // Assumes a query `_getCategoryNameById` exists on CategoryConcept.
    ...
    if (frames.length === 0) {
      return new Frames({ ...originalFrame, [error]: "Category not found" });
    }

    return frames;
  },
  then: actions([Requesting.respond, { request, name, error }]),
});
...
export const GetCategoriesFromOwnerRequest: Sync = (
  ...
  where: async (frames) => {
    ...
    frames = await frames.query(
      Category._getCategoriesFromOwner,
      { owner_id: user },
      { category_id, name },
    );
    if (frames.length === 0) {
...
export const GetMetricRequest: Sync = (
  { request, session, user, category_id, startDate, endDate, period, metric, error },
) => ({
  when: actions([Requesting.request, {
  ...
  where: async (frames) => {
    ...
    let periodObj;
    try {
      periodObj = Period.from(new Date(frame[startDate]), new Date(frame[endDate]));
    } catch (e) {
