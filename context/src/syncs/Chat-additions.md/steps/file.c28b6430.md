...
content_id: c28b6430d7d9b6269f49ab71dda08cf0aa6354bec0168ef6ceb3bbeb066b4511
---
...
I have replaced the previous implementation of `GetMetricStatsRequest` with a new version. However, for this to work perfectly, a small change in the concept is highly recommended to align with standard practice.

### Recommended Change in `CategoryConcept.ts`

For maximum clarity and consistency with the concept design pattern, the `getMetricStats` query should wrap its output in a named key that matches the binding in the sync.

In `src/concepts/Category/CategoryConcept.ts`, change the `return` statements in the `getMetricStats` method:
...
//...
    return [{ stats: { total_amount, transaction_count, average_per_day, days } }];
```

By making this small change, the concept's output now has the `stats` key that the reworked `GetMetricStatsRequest` sync is expecting, resolving the "Missing binding" error and allowing the request to be handled correctly.
