...
content_id: 6ba544e16384cb5193f058c9016dabf4f1a981a9e0d4ad359124e3fed2125d23
---
...
Deno.test("Principle: Staging and Finalizing with conflicts", async () => {
  ...
  try {
    ...
    await (store as any)._commitSingleLabel(
      user,
      txA_id,
      txA_name,
      txA_merchant,
    ...
    assertExists(
      labelA,
      "txA_id should be committed (from manual setup commit)",
    );
    ...
    assertEquals(
      labelB,
      null,
      "txB_id should NOT be committed as finalize failed (all-or-nothing).",
    );
...
Deno.test("Principle: User cancels staged labels", async () => {
  ...
  try {
    ...
    assertEquals(
      await store.getLabel(tx1_id),
      null,
      "Committed label for tx1 should still be null after cancel.",
    );
    assertEquals(
      await store.getLabel(tx2_id),
      null,
      "Committed label for tx2 should still be null after cancel.",
    );

    // Verify no transaction info after cancel
    assertEquals(
      await store.getTxInfo(tx1_id),
      null,
      "Tx info for tx1 should still be null after cancel.",
    );
    assertEquals(
      await store.getTxInfo(tx2_id),
      null,
      "Tx info for tx2 should still be null after cancel.",
    );

    // Verify no category history entry after cancel
    assertEquals(
      (await store.getCategoryHistory(catGroceries)).length,
      0,
      "Category history should be empty after cancel.",
    );
