---
timestamp: 'Sat Oct 18 2025 17:43:40 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251018_174340.15f1e60f.md]]'
content_id: 1531c115d0ae479f2a7677ce52d14ec9424c7711d32377207343d4762bc16d4a
---
...
export class Period {
  ...
  static from(startDate: Date, endDate: Date): Period {
    return new Period(startDate, endDate); // Constructor already validates
  }
...
export class CategoryStore {
  ...
  constructor(private readonly db: Db) {
    ...
    // It is common practice to initialize indexes in the constructor for development/local environments.
    // In a production environment, index creation might be handled as part of migration scripts.
    this.initializeIndexes();
  }

  /**
   * Initializes unique indexes for the collections.
   * `createIndex` is idempotent, so it's safe to call on startup.
   */
  private async initializeIndexes() {
    // Unique compound index for categories on (owner_id, name)
    await this.categories.createIndex({ owner_id: 1, name: 1 }, { unique: true });

    // Unique compound index for metrics on (owner_id, category_id, period_start, period_end)
    await this.categoryMetrics.createIndex(
      { owner_id: 1, category_id: 1, period_start: 1, period_end: 1 },
      { unique: true },
    );
  }
  ...
  async create(owner_id: Id, name: string): Promise<{ category_id: Id }> {
    ...
    // The unique index also enforces this, but an explicit check provides a cleaner error message.
  ...
  async rename(
    ...
    // An explicit check below reinforces the principle, though might be redundant given _id design.
  ...
  /**
   * Sets the total for a CategoryMetric for a given period.
   * Requires that the category exists and total is non-negative.
   * This action is idempotent for the same inputs.
   */
  ...
  async delete(
    ...
    // Requires: can_delete = true (only called by the sync which gets result from label's)
    if (!can_delete) {
      throw new Error(
    ...
    // 1. Remove associated CategoryMetrics first using the dedicated method
    await this.deleteMetricsForCategory(
      owner_id,
      category_id,
    );

    // 2. Remove CategoryDoc
