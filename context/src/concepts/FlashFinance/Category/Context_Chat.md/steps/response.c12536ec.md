---
timestamp: 'Sat Oct 18 2025 18:29:36 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251018_182936.bb83416a.md]]'
content_id: c12536ece7ccec0d5337e79cdaf359fd9f1ac3b7af9b8fa1a26a5894c5a87c4f
---
...
Deno.test("CategoryStore: delete action works and blocks when referenced by labels", async () => {
  ...
  try {
    ...
    // As per prompt, set has_labels_in_category to true for normal deletion (means it CAN be deleted)
...
Deno.test("CategoryStore: CategoryMetrics actions (set, get, list, delete with category) work as expected", async () => {
  ...
  try {
    ...
    let metricsListAfterUpdate = await store.listMetrics(ownerId, categoryId1);
    assertEquals(
    ...
    assertEquals(allMetrics.length, 3, "Should have 3 metrics now (Jan, Feb, Mar).");
    ...
    console.log("   ✅ Verified: listMetrics returns all metrics sorted by period_start ascending.");
    ...
    console.log("   ✅ Verified: Setting metric for non-existent category rejected.");
    ...
    let categoriesAfterDelete = await store.getCategoryNamesAndOwners();
    assertEquals(
