---
timestamp: 'Tue Nov 04 2025 15:57:54 GMT-0500 (Eastern Standard Time)'
parent: '[[..\20251104_155754.4afa39fa.md]]'
content_id: ffd7cb63190e59926ede6600f849a028e81ddfa24b255179d2749ac58c4d39df
---
...
export default class UserAuthenticationConcept {
  ...
  /**
   * changePassword (username: String, oldPassword: String, newPassword: String): ({}) | (error: String)
   *
   * **requires**: a User exists with the given `username` and the `oldPassword` matches their `passwordHash`.
   * **effects**: updates the User's `passwordHash` to a hash of the `newPassword`; returns an empty object on success.
   *
   * **requires**: no User exists with the given `username` or the `oldPassword` does not match.
   * **effects**: returns an error message.
   *
   * **requires**: the `newPassword` is the same as the `oldPassword`.
   * **effects**: returns an error message.
   */
  async changePassword({ username, oldPassword, newPassword }: { username: string; oldPassword: string; newPassword: string }): Promise<Record<string, never> | { error: string }> {
    const user = await this.users.findOne({ username });

    // To prevent timing attacks and username enumeration, use a generic error message for both non-existent user and wrong password.
    if (!user) {
      return { error: "Invalid username or password" };
    }
    ...
    await this.users.updateOne({ _id: user._id }, { $set: { passwordHash: newPasswordHash } });
