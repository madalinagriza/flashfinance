...
export default class LeaderboardConcept {
  ...
  async createPlayer(
    ...
    const maybe_exists = await this.players.findOne({ name });

    if (maybe_exists) {
      return { error: "Player with this name already exists" };
    }
    const newPlayerId = freshID();
    await this.players.insertOne({ _id: newPlayerId, name });
  ...
  async submitScore(
    ...
    const newScoreId: Score = freshID();
    ...
    await this.scores.insertOne(newScore);
