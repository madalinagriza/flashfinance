...
// as it is not exported from the main LabelConcept.ts module.
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      let initialLabels = await store.getLabel(user, tx1_id);
      assertEquals(
      ...
      // getLabel now takes user_id
      initialLabels = await store.getLabel(user, tx1_id);
      assertEquals(
        initialLabels.length,
        1,
        "Label should be created after 'finalize'.",
      );
      const [initialLabel] = initialLabels;
      ...
      assertExists(
        txInfo,
        "Transaction info should be saved after 'finalize'.",
      );
      ...
      // getLabel now takes user_id
      const updatedLabels = await store.getLabel(user, tx1_id);
      assertEquals(
        updatedLabels.length,
        1,
        "Updated label should exist for the transaction.",
      );
      const [updatedLabel] = updatedLabels;
      assertEquals(
      ...
      await store.removeCommittedLabel(user, tx1_id);

      // 9. VERIFY a) The label's category_id is now the special TRASH_CATEGORY_ID.
      // getLabel now takes user_id
      const removeCommittedLabeldLabelEntries = await store.getLabel(
        user,
        tx1_id,
      );
      assertEquals(
        removeCommittedLabeldLabelEntries.length,
        1,
        "Removing the label should result in a Trash entry.",
      );
      const [removeCommittedLabeldLabel] = removeCommittedLabeldLabelEntries;
      assertEquals(
        removeCommittedLabeldLabel.category_id,
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      await store.cancelSession(userA); // Clean up staged labels for userA
      console.log(
      ...
      const committedA_tx2Entries = await store.getLabel(userA, tx2_id);
      assertEquals(
        committedA_tx2Entries.length,
        1,
        "Tx2 should be committed for userA after finalize.",
      );
      const [committedA_tx2] = committedA_tx2Entries;
      assertEquals(committedA_tx2.user_id, userA.toString());

      const committedA_tx3Entries = await store.getLabel(userA, tx3_id);
      assertEquals(
        committedA_tx3Entries.length,
        1,
        "Tx3 should be committed for userA after finalize.",
      );
      const [committedA_tx3] = committedA_tx3Entries;
      ...
      const committedB_tx2Entries = await store.getLabel(userB, tx2_id);
      assertEquals(
        committedB_tx2Entries.length,
        1,
        "Tx2 should remain committed for userB.",
      );
      const [committedB_tx2] = committedB_tx2Entries;
      ...
      // getLabel now takes user_id
      const committedTx2Entries = await store.getLabel(userB, tx2_id);
      assertEquals(
        committedTx2Entries.length,
        1,
        "Tx2 should be committed by userB.",
      );
      const [committedTx2] = committedTx2Entries;
      assertEquals(
      ...
      await store.cancelSession(userA); // Clear userA's remaining staged labels
      console.log(
      ...
      // getLabel now takes user_id
      let tx4CommittedLabels = await store.getLabel(userA, tx4_id);
      assertEquals(
        tx4CommittedLabels.length,
        0,
        "Tx4 should not be committed before cancel.",
      );
      let tx5CommittedLabels = await store.getLabel(userA, tx5_id);
      assertEquals(
        tx5CommittedLabels.length,
        0,
        "Tx5 should not be committed before cancel.",
      );

      console.log("   Staged labels. Now cancelling...");
      await store.cancelSession(userA);

      // Verify no committed labels after cancel
      // getLabel now takes user_id
      tx4CommittedLabels = await store.getLabel(userA, tx4_id);
      assertEquals(
        tx4CommittedLabels.length,
        0,
        "Committed label for tx4 should still be null after cancel.",
      );
      tx5CommittedLabels = await store.getLabel(userA, tx5_id);
      assertEquals(
        tx5CommittedLabels.length,
        0,
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const initialCategoryHistoryAfterFinalize = await store
        .getCategoryHistory(
          user,
          categoryId1,
        );
